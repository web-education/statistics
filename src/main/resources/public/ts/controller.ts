...
export const statsController = ng.controller('StatsController', ['$scope', '$timeout', 'model',
	...
	$scope.getExportUrl = (indicator: IndicatorApi) => {
		return encodeURI(`/stats/export?indicator=${indicator}&from=${dateService.getSinceDateISOStringWithoutMs()}&frequency=day&entityLevel=${$scope.scopeEntity.current.level}&entity=${$scope.scopeEntity.current.id}`);
	};
